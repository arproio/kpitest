CREATE OR REPLACE FUNCTION public.asset_kpi_full(value_stream_name character varying, machine_id character varying, end_time_string character varying, duration_in_sec integer, idea_run_rate integer)
 RETURNS record
 LANGUAGE plpgsql
 STABLE
AS $function$
...
begin
    ...
    IF duration_in_sec <=0 then
		...
		0 as EFFICIENCY,
        var_start_time as STARTTIME,
        var_end_time as ENDTIME,

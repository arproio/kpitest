CREATE OR REPLACE FUNCTION public.asset_statetimesum_batch_realtime(dailyshiftid character varying, shiftid character varying, shiftname character varying, start_time_string character varying, end_time_string character varying, machine_ids character varying[], value_streams character varying[], streams character varying[], par_property_names character varying[], expected_values integer[], target_names character varying[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
begin
	...
	LOOP
		...
		perform public.asset_statetimesum_realtime(var_current_value_stream, var_current_stream, machine_id, var_current_property_name, var_current_expected_value, var_current_target_name, start_time_string, end_time_string, shiftname);

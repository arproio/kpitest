...
--    into var_efficiency_return
--    from value_stream
--    where entity_id=value_stream_name and source_id = machine_id and property_name='param_utillizationtotal' and property_value <> 999
--	and time < var_end_time and time > var_start_time
--    
--    if var_efficiency_return is null then
--	    	if var_utillizationtotal_last is null then
--	    		var_efficiency_return := 0.0;
--	    	else
--	    		var_efficiency_return := var_utillizationtotal_last;
--	    	end if;
--    end if;
	...
	if availability_window <= 0.0 then
		...
		LOOP
			...
			if var_last_alarm_val = 'true' then
				available_alarmtime_in_sec := extract(epoch from rec.time - var_last_time) + available_alarmtime_in_sec;
			end if;

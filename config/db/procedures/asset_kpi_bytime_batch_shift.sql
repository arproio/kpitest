CREATE OR REPLACE FUNCTION public.asset_kpi_bytime_batch_shift(dailyshiftid character varying, shiftid character varying, shiftname character varying, start_time_string character varying, end_time_string character varying, ideal_run_rate integer, machine_ids character varying[], machine_types character varying[], value_streams character varying[], streams character varying[], countable_machine_ids character varying[])
...
--		
		for myTOTALCOUNT, myREJECTCOUNT in
			...
			from asset_kpi_bytime_shift(var_current_value_stream, var_current_stream, machine_id, var_current_machine_type, start_time_string, end_time_string, ideal_run_rate, shiftname, dailyshiftid) onemachine
		LOOP

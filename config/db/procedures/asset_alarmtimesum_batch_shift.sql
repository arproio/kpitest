CREATE OR REPLACE FUNCTION public.asset_alarmtimesum_batch_shift(dailyshiftid character varying, shiftid character varying, shiftname character varying, kpi_date character varying, start_time_string character varying, end_time_string character varying, machine_ids character varying[], machine_types character varying[], value_streams character varying[], streams character varying[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
...
BEGIN
	...
	LOOP
		...
		perform public.asset_alarmtimesum_shift(var_current_value_stream, var_current_stream, machine_id, var_current_properties, start_time_string, end_time_string, shiftname);
